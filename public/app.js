...
const VARIANZAS_KEYS = [
  { key: 'u_patron',        label: 'Patron (÷2)',          div: 2             },
  { key: 'u_resolucion',    label: 'Resolución (÷√12)',    div: Math.sqrt(12) },
  { key: 'u_excentricidad', label: 'Excentricidad (÷√3)',  div: Math.sqrt(3)  },
  { key: 'u_deriva',        label: 'Deriva (÷√3)',          div: Math.sqrt(3)  },
  { key: 'u_rr',            label: 'R&R (÷√3)',             div: Math.sqrt(3)  },
  { key: 'u_sensibilidad',  label: 'Sensibilidad (÷√3)',    div: Math.sqrt(3)  },
  { key: 'u_temperatura',   label: 'Temperatura (÷√3)',     div: Math.sqrt(3)  },
  { key: 'u_densidad',      label: 'Densidad (÷√3)',        div: Math.sqrt(3)  },
  { key: 'u_flotabilidad',  label: 'Flotabilidad (÷√3)',    div: Math.sqrt(3)  },
  { key: 'u_labmaster',     label: 'Labmaster (sin ÷)',     div: 1             },
];
...
// Ej: 0.000082 → "0.000082", 1.2345 → "1.2", 120 → "120", 0.0012 → "0.0012"
function fmtSig(num) {
  if (num === null || num === undefined || !isFinite(num)) return '—';
  if (num === 0) return '0';
  const val = cifrasSig(num, 2);
  const exp = Math.floor(Math.log10(Math.abs(val)));
  // Decimales necesarios = max(0, 1 - exp) asegura 2 cifras sig
  // Ej: exp=0 (val~1.x) → toFixed(1); exp=1 (val~1x) → toFixed(0) pero necesita 2 sig → especial
  if (exp >= 1) {
    // Número >= 10: sin decimales (ej 120, 8500)
    return Math.round(val).toString();
  } else if (exp === 0) {
    // Número en [1,10): siempre 1 decimal para mostrar 2 cifras sig
    return val.toFixed(1);
  } else {
    // Número < 1: suficientes decimales para mostrar 2 cifras sig
...
function renderPuntoRow(p, idx, eqId, magId, tipo) {
  ...
  const varianzasObj = p.varianzas || {};
  const activeCount  = VARIANZAS_KEYS.filter(vk => {
    const v = parseFloat(varianzasObj[vk.key]);
    return !isNaN(v) && v > 0;
  }).length;

  const varInputs = VARIANZAS_KEYS.map(vk => {
    const rawVal = varianzasObj[vk.key];
    const val    = rawVal !== undefined && rawVal !== '' ? esc(String(rawVal)) : '';
    const numVal = parseFloat(rawVal);
    const active = !isNaN(numVal) && numVal > 0;
    // Mostrar la contribución ui = val/div en tiempo real
    const contrib = active
      ? `<span class="varianza-contrib" title="Contribución: ui = ${rawVal}/${vk.div.toFixed(4)}">
           ui = ${(numVal / vk.div).toExponential(2)}
         </span>`
      : '';
    return `<div class="varianza-item${active ? ' varianza-active' : ''}" data-vkey="${vk.key}">
  ...
  const activeBadge = activeCount > 0
    ? `<span class="var-active-badge">${activeCount}</span>` : '';

  const varCell = `<td>
    <div class="varianzas-toggle" onclick="toggleVarianzas('var-${p.id}',this)">
      <span id="var-chv-${p.id}">&#9654;</span> Varianzas ${activeBadge}
    </div>
    <div class="varianzas-panel" id="var-${p.id}" style="display:none">
      ${varInputs}
    </div>
  </td>`;
...
function updateVarianza(eqId, magId, puntoId, vkey, value) {
  ...
  guardar();
  actualizarCeldasResultado(p);
  // Actualizar contribuciones ui en el panel sin re-renderizar
  actualizarContribuciones(p, puntoId);
}

function actualizarContribuciones(p, puntoId) {
  const tr = document.querySelector(`[data-punto="${puntoId}"]`);
  if (!tr) return;
  const varianzasObj = p.varianzas || {};
  let activeCount = 0;
  VARIANZAS_KEYS.forEach(vk => {
    const rawVal = varianzasObj[vk.key];
    const numVal = parseFloat(rawVal);
    const active = !isNaN(numVal) && numVal > 0;
    if (active) activeCount++;
    // Actualizar o insertar la contribución
    const item = tr.querySelector(`[data-vkey="${vk.key}"]`);
    if (item) {
      item.classList.toggle('varianza-active', active);
      let contrib = item.querySelector('.varianza-contrib');
      if (active) {
        const ui = numVal / vk.div;
        if (!contrib) {
          contrib = document.createElement('span');
          contrib.className = 'varianza-contrib';
          item.appendChild(contrib);
        }
        contrib.title = `Contribución: ui = ${rawVal}/${vk.div.toFixed(4)}`;
        contrib.textContent = `ui = ${ui.toExponential(2)}`;
      } else if (contrib) {
        contrib.remove();
      }
    }
  ...
  const toggle = tr.querySelector('.varianzas-toggle');
  if (toggle) {
    let badge = toggle.querySelector('.var-active-badge');
    if (activeCount > 0) {
      if (!badge) {
        badge = document.createElement('span');
        badge.className = 'var-active-badge';
...
function toggleVarianzas(id, toggle) {
  ...
  if (chv) chv.style.transform = isOpen ? '' : 'rotate(90deg)';
}
